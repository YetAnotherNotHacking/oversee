...
def crawl_all_pages():
    ...
    for page_num in range(1, total_pages + 1):
        ...
        print(f"Crawling page {page_num}/{total_pages}: {page_url}"); links = extract_stream_links(page_url); total_links += len(links)
...
def scrape_insecam_camera_urls(output_file="stream_links.txt", base_url = "http://www.insecam.org/en/byrating/", total_pages = 448):
    ...
    if os.path.exists(output_file):
        ...
        print(f"\nCrawling completed!"); print(f"Total time elapsed: {elapsed_time:.2f} seconds"); print(f"Total links saved: {line_count}"); print(f"Output file: {output_file} ({file_size} bytes)")
    else:
