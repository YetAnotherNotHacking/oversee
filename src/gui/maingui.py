...
class MainGUI:
    ...
    def create_camguessr_tab(self):
        """Create the CamGuessr tab with map view and controls"""
        camguessr_frame = ttk.Frame(self.main_notebook)
        self.main_notebook.add(camguessr_frame, text="CamGuessr")
        
        # Configure grid for camguessr frame
        camguessr_frame.grid_rowconfigure(0, weight=1)
        camguessr_frame.grid_columnconfigure(0, weight=2)  # Map takes 2/3 of width
        camguessr_frame.grid_columnconfigure(1, weight=1)  # Preview takes 1/3 of width
        
        # Create top bar for login status
        top_bar = ttk.Frame(camguessr_frame)
        top_bar.grid(row=0, column=0, columnspan=2, sticky="ew", padx=10, pady=5)
        
        # Add login status label
        self.login_status_label = ttk.Label(top_bar, text="Not logged in")
        self.login_status_label.pack(side="right", padx=5)
        
        # Add login/logout button
        self.login_button = ttk.Button(top_bar, text="Login", command=self.show_login_dialog)
        ...
        try:
            ...
            # Create the map widget (left side)
            map_widget = TkinterMapView(camguessr_frame, width=800, height=600, corner_radius=0)
            map_widget.grid(row=1, column=0, sticky="nsew", padx=10, pady=10)
            
            # Set initial position and zoom
            map_widget.set_position(0, 0)
            map_widget.set_zoom(2)
            
            # Create camera preview frame (right side)
            preview_frame = ttk.Frame(camguessr_frame)
            preview_frame.grid(row=1, column=1, sticky="nsew", padx=10, pady=10)
            preview_frame.grid_rowconfigure(0, weight=1)
            preview_frame.grid_columnconfigure(0, weight=1)
            
            # Add camera preview label
            self.camguessr_preview_label = ttk.Label(preview_frame, text="Loading camera...")
            self.camguessr_preview_label.grid(row=0, column=0, sticky="nsew", padx=10, pady=10)
            
            # Add vote button frame
            vote_frame = ttk.Frame(preview_frame)
            vote_frame.grid(row=1, column=0, sticky="ew", padx=10, pady=5)
            
            # Add vote button
            self.vote_button = ttk.Button(vote_frame, text="Vote New Camera", command=self.vote_new_camera)
            self.vote_button.pack(side="left", padx=5)
            
            # Add vote count label
            self.vote_count_label = ttk.Label(vote_frame, text="Votes: 0/3")
            self.vote_count_label.pack(side="left", padx=5)
            
            # Initialize vote tracking
            self.current_votes = 0
            self.required_votes = 3  # Majority threshold
            self.vote_timer = None
            
            # Create control panel at the bottom
            control_frame = ttk.Frame(camguessr_frame)
            ...
            # Add score display
            score_label = ttk.Label(control_frame, text="Score: 0")
            score_label.pack(side="right", padx=10)
            
            # Add round counter
            round_label = ttk.Label(control_frame, text="Round: 1/5")
            round_label.pack(side="right", padx=10)
            
            # Set initial map style
            change_map_style()
            
            # Start camera preview
            self.start_camguessr_preview()
    ...
    def show_login_dialog(self):
        ...
        login_window.title("CamGuessr Login")
        ...
        login_window.geometry(f"300x400+{x}+{y}")
        ...
        # Add title
        title_label = ttk.Label(main_frame, text="CamGuessr Login", font=('Arial', 14, 'bold'))
        ...
        # Add verification code entry (initially hidden)
        code_frame = ttk.Frame(main_frame)
        code_var = tk.StringVar()
        
        def show_code_entry():
            code_frame.pack(fill="x", pady=(0, 20))
            verify_button.pack(fill="x", pady=(0, 10))
            ...
            def registration_worker():
                try:
                    response = requests.post(
                        "http://silverflag.net:8000/api/register",
                        data={
                            "username": username,
                            "email": email
                        }
                    ...
                    if response.status_code == 200:
                        self.root.after(0, lambda: messagebox.showinfo("Success", "Registration successful! Please check your email for verification code."))
                        self.root.after(0, lambda: show_verification_dialog())
                    else:
                    ...
                    self.root.after(0, lambda: register_button.config(state="normal", text="Register"))
        ...
        def login():
            try:
                response = requests.post(
                    "http://silverflag.net:8000/api/login",
        ...
        def verify():
            try:
                response = requests.post(
                    "http://silverflag.net:8000/api/verify",
                    data={
                        "email": email_var.get(),
                        "code": code_var.get()
                    }
        ...
        verify_button = ttk.Button(main_frame, text="Verify", command=verify)
        ...
        note_label = ttk.Label(
            main_frame,
            text="Note: A verification code will be sent to your email.\nNo password required!",
            justify="center",
            wraplength=250
        )
        ...
        if self.session_token:
            try:
                response = requests.get(
                    "http://silverflag.net:8000/api/user",
                    params={"session_token": self.session_token}
                )
                if response.status_code == 200:
                    user_data = response.json()
                    self.login_status_label.config(text=f"Logged in as: {user_data['username']}")
    ...
    def logout(self):
        ...
        if self.session_token:
            try:
                requests.post(
                    "http://silverflag.net:8000/api/logout",
    ...
    def start_camguessr_preview(self):
        """Start the camera preview for CamGuessr"""
        try:
            # Get a random working camera from the database
            conn = self.get_thread_db_connection()
            cursor = conn.cursor()
            cursor.execute('SELECT ip FROM cameras WHERE status = "Online" ORDER BY RANDOM() LIMIT 1')
            result = cursor.fetchone()
            conn.close()
            ...
            if result:
                ...
                threading.Thread(
                    target=self.camguessr_preview_worker,
                    args=(ip,),
                    daemon=True
                ).start()
            else:
                self.camguessr_preview_label.config(text="No working cameras found")
        except Exception as e:
            print(f"Error starting CamGuessr preview: {e}")
            self.camguessr_preview_label.config(text="Error loading camera")

    def camguessr_preview_worker(self, ip):
        """Worker thread for CamGuessr camera preview"""
        try:
            ...
            if test_frame is None:
                self.camguessr_preview_label.after(0, lambda: self.camguessr_preview_label.config(
                    text="Camera not responding"
                ))
                return
            ...
            while True:  # Run until the window is closed
                try:
                    with frame_lock:
                        ...
                        if frame is not None:
                            ...
                            self.camguessr_preview_label.after(0, lambda p=photo: self.update_camguessr_preview(p))
                        else:
                    ...
                    print(f"Error in CamGuessr preview loop: {str(e)}")
                    self.camguessr_preview_label.after(0, lambda: self.camguessr_preview_label.config(
            ...
            print(f"Fatal error in CamGuessr preview: {str(e)}")
            self.camguessr_preview_label.after(0, lambda: self.camguessr_preview_label.config(
                text=f"Camera error: {str(e)}"
            ))

    def update_camguessr_preview(self, photo):
        """Update CamGuessr preview image in main thread"""
        try:
            self.camguessr_preview_label.config(image=photo, text='')
            self.camguessr_preview_label.image = photo  # Keep a reference to prevent garbage collection
        except tk.TclError:
            pass

    def vote_new_camera(self):
        """Handle voting for a new camera"""
        # Increment vote count
        self.current_votes += 1
        self.vote_count_label.config(text=f"Votes: {self.current_votes}/{self.required_votes}")
        
        # If we have enough votes, change the camera
        if self.current_votes >= self.required_votes:
            self.current_votes = 0
            self.vote_count_label.config(text=f"Votes: {self.current_votes}/{self.required_votes}")
            self.start_camguessr_preview()
        else:
            # Start a 30-second timer to reset votes if not enough votes are collected
            if self.vote_timer:
                self.root.after_cancel(self.vote_timer)
            self.vote_timer = self.root.after(30000, self.reset_votes)
    
    def reset_votes(self):
        """Reset the vote count after timeout"""
        self.current_votes = 0
        self.vote_count_label.config(text=f"Votes: {self.current_votes}/{self.required_votes}")
        self.vote_timer = None
